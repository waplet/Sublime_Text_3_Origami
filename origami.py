...
class PaneCommand(sublime_plugin.WindowCommand):
	...
	def zoom_pane(self, fraction, widen=True):
		if fraction == None:
			fraction = .1
		...
		if widen:
			new_size = cols[current_col+1]-cols[current_col] + fraction
		else:
			new_size = cols[current_col+1]-cols[current_col] - fraction

		new_size = min(1, max(0, new_size));
		...
		current_col_width = 1 if num_cols==1 else new_size
		...
		# sublime.status_message(str(cols));
...
class ZoomPaneCommand(PaneCommand):
	def run(self, fraction=None, widen=True):
		self.zoom_pane(fraction, widen)
